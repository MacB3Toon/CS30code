...
function updateGrid(){
  ...
  for (let y = 0; y < ROWS; y++){
    for (let x = 0; x< COLS; x ++){ 
      ...
      //be careful about counting self cell...
      neighbours -= grid[y][x];

      //apply rules
      if (grid[y][x] ===  1){ //alive
        if (neighbours === 2 || neighbours === 3){
          nextTurn[y][x] = 1; //stayin' alive
        }
        else{
          nextTurn[y][x] = 0; //dead from loneliness or overpopulation
        }
      }
      if (grid[y][x] ===  0){ //dead
        if (neighbours === 3){
          nextTurn[y][x] = 1; //being birthed
        }
        else{
          nextTurn[y][x] = 0; //dead from being dead
        }
      }
    }
  }
